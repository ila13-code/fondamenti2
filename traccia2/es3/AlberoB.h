...
public:
    ...
    // inserisce un nodo foglia in d= SIN/DES con contenuto informativo=info
    void insNodo(Direzione d, T info)
    {
        assert( !nullo() );
        {
            AlberoB mioB(info);
            pradice->pfiglio[d]=mioB.pradice;
            mioB.pradice->ppadre=pradice;
        }
    };
    ...
    int Nodofiglio ( Direzione d ) const {
        assert( !nullo() );
        AlberoB<T> AC;
        AC.pradice = pradice->pfiglio[d];
        return pradice->pfiglio[d]->vinfo;
    };
